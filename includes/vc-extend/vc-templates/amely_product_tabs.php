...
<div class="<?php echo esc_attr( trim( $css_class ) ); ?>" id="<?php echo esc_attr( $css_id ); ?>"
	...
	<ul class="product-filter">
		<?php if ( $filter == 'category' ) { ?>
			...
			<?php foreach ( $categories as $index => $category ) { ?>
				<li><a href="#" class="<?php echo ( $filter_type == 'ajax' && ! $index ) ? 'active' : '' ?>"
				       data-page="<?php echo esc_attr( 1 ); ?>"
				       data-category="<?php echo esc_attr( $category->slug ); ?>"
				       data-filter="<?php echo esc_attr( '.product_cat-' . $category->slug ) ?>"><?php echo( $category->name ); ?></a>
				</li>
			<?php } ?>
		<?php } else { ?>
			<?php if ( $filter_type == 'filter' ) { ?>
	...
	if ( $pagination_type ) {
		...
		?>
		<div class="amely-loadmore-wrap" data-filter="<?php echo esc_attr( $categories[0]->slug ); ?>"
