...
switch ( $source ) {
	...
	case 'external_link':
		...
		$img = array(
			'thumbnail' => '<img ' . $hwstring . 'src="' . $custom_src . '" alt="' . esc_attr('image') . '" />',
			'src'       => $custom_src,
		);
...
<div class="<?php echo esc_attr( trim( $css_class ) ); ?>" id="<?php echo esc_attr( $css_id ); ?>">
	<a class="banner-image"
	   href="<?php echo esc_url( $a_href ); ?>"
		<?php echo !empty($a_title) ? 'title="' . esc_attr( $a_title ) . '"' : ''; ?>
		<?php echo !empty($a_target) ? 'target="' . esc_attr( $a_target ) . '"' : ''; ?>
		<?php echo !empty($a_rel) ? 'rel="' . esc_attr( $a_rel ) . '"' : ''; ?>><?php echo $a_title ? esc_html( $a_title ) : esc_html__( 'View Details',
			'amely' ); ?>
		<?php if ( $hashtag ) { ?>
		...
		<?php if ( $button_visibility != 'hidden' ) { ?>
			<a class="amely-button banner-button <?php echo esc_attr( $button_style ); ?>"
			   href="<?php echo esc_url( $a_href ); ?>"
				<?php echo !empty($a_title) ? 'title="' . esc_attr( $a_title ) . '"' : ''; ?>
				<?php echo !empty($a_target) ? 'target="' . esc_attr( $a_target ) . '"' : ''; ?>
				<?php echo !empty($a_rel) ? 'rel="' . esc_attr( $a_rel ) . '"' : ''; ?>>
