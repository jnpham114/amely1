...
if ( ! class_exists( 'Amely_Woo' ) ) {
	...
	class Amely_Woo {
		...
		public static function wishlist_button() {
			if ( class_exists( 'YITH_WCWL' ) && amely_get_option( 'shop_wishlist_on' ) ) {
		...
		public static function compare_button() {
			...
			if ( ( class_exists( 'WPCleverWoosc' ) || class_exists( 'WooSCP' ) ) && amely_get_option( 'shop_compare_on' ) ) { ?>
				<div class="compare-btn hint--bounce hint--top-left"
				     aria-label="<?php esc_html_e( 'Compare', 'amely' ); ?>">
					<?php echo do_shortcode( '[woosc id="' . $id . '" type="link"]' ); ?>
				</div>
							...
							<?php foreach ( $media_array as $item ) { ?>
								<div class="item <?php echo esc_attr( $class ) ?>">
									...
									<img src="<?php echo esc_url( $item['thumbnail'] ) ?>"
									     alt="<?php esc_attr_e( 'image', 'amely' ); ?>"
									     class="item-image"/>
		...
		public static function get_products_by_datasource( $data_source, $atts, $args = array() ) {
			...
			switch ( $data_source ) {
				...
				case 'products':
					...
					break;
				case 'categories':
					if ( ! empty( $atts['product_cat_slugs'] ) ) {
						$args['tax_query'] = array(
							'relation' => 'OR',
							array(
								'taxonomy'         => 'product_cat',
		...
		public function admin_notice_minimum_compare_plugin_version() {
			amely_notice_required_plugin_version( 'WPC Smart Compare for WooCommerce', self::MINIMUM_PLUGIN_VERSION );
		}
	}
