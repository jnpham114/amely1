...
if ( ! class_exists( 'Amely_Enqueue' ) ) {
	...
	class Amely_Enqueue {
		...
		public function enqueue_libs() {
			...
			if ( defined( 'YITH_WCWL' ) && ! function_exists( 'yith_wcwl_dequeue_styles' ) ) {
				function yith_wcwl_dequeue_styles() {
					wp_deregister_style( 'yith-wcwl-font-awesome' );
				}
				add_action( 'wp_enqueue_scripts', 'yith_wcwl_dequeue_styles', 5 );
			}
...
//				wp_enqueue_style( 'font-awesome', AMELY_THEME_URI . '/assets/libs/font-awesome/css/font-awesome.min.css' );
			}
			wp_enqueue_style( 'font-awesome', AMELY_THEME_URI . '/assets/libs/font-awesome/css/font-awesome.min.css' );
			...
			if ( class_exists( 'InsightCore' ) ) {
				function dequeue_insightcore_hint_css() {
					wp_dequeue_style('hint-css');
				}
				add_action('wp_enqueue_scripts','dequeue_insightcore_hint_css');
			}
		}
		...
		public function enqueue() {
			 ...
			 */
			if ( is_rtl() ) {
				wp_enqueue_style( 'amely-main-style', get_template_directory_uri() . '/style-rtl' . ( amely_get_option( 'minified_on' ) ? '.min' : '' ) . '.css' );
				wp_enqueue_style( 'amely-style-rtl-custom', get_template_directory_uri() . '/style-rtl-custom' . ( amely_get_option( 'minified_on' ) ? '.min' : '' ) . '.css' );
			} else {
				wp_enqueue_style( 'amely-main-style', get_template_directory_uri() . '/style' . ( amely_get_option( 'minified_on' ) ? '.min' : '' ) . '.css' );
			}
			...
			wp_localize_script( 'amely-main-js',
				...
				array(
					...
					'zoomEnable'                       => amely_get_option( 'product_zoom_on' ),
					'isRTL'                            => AMELY_IS_RTL,
				) );
		}
