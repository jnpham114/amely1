	...
	<div id="comments">
		<h2 class="woocommerce-Reviews-title">
			...
			if ( $count && wc_review_ratings_enabled() ) {
				...
				$reviews_title = sprintf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'amely' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
				echo apply_filters( 'woocommerce_reviews_title', $reviews_title, $count, $product ); // WPCS: XSS ok.
			} else {
				esc_html_e( 'Reviews', 'amely' );
			}
			?>
		</h2>
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php esc_html_e( 'There are no reviews yet.', 'amely' ); ?></p>
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				...
				$comment_form = array(
					...
					'label_submit'        => esc_html__( 'Submit', 'amely' ),
					'logged_in_as'        => '',
					'comment_field'       => '',
				);
