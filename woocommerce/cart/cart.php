				...
				<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class',
					...
					<td class="product-name" data-title="<?php esc_html_e( 'Product', 'amely' ); ?>">
						...
						if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
							echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'amely' ) . '</p>', $product_id ) );
						}
						?>
					</td>
		...
		<div class="col-md-9 col-lg-8">
			<?php if ( wc_coupons_enabled() ) { ?>
				<div class="coupon">
					...
					<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'amely' ); ?>" />
					<button type="submit" class="button" name="apply_coupon" value="<?php esc_attr_e( 'Apply coupon', 'amely' ); ?>"><?php esc_html_e( 'Apply coupon', 'amely' ); ?></button>
					<?php do_action( 'woocommerce_cart_coupon' ); ?>
				</div>
			<?php } ?>
		</div>
		<div class="col-md-3 col-lg-4">
			<button type="submit" class="button" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'amely' ); ?>"><?php esc_html_e( 'Update cart', 'amely' ); ?></button>
